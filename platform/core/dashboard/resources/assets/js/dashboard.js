            ...
            success: res =>  {
                ...
                if (!res.error) {
                    ...
                    if (el.find('.scroller').length !== 0) {
                        Platform.callScroll(el.find('.scroller'));
                    }
            ...
            Sortable.create(el, {
                ...
                onEnd: () => {
                    ...
                    $.ajax({
                        ...
                        success: res =>  {
                            if (!res.error) {
                                Platform.showSuccess(res.message);
                            } else {
                                Platform.showError(res.message);
                            }
                        },
                        error: data =>  {
    ...
    init() {
        ...
        list_widgets.on('change', '.number_record .numb', event =>  {
            ...
            if (!isNaN(paginate)) {
                ...
                Platform.showError('Please input a number!')
            }
        ...
        $(document).on('click', '.portlet:not(.widget-load-has-callback) > .portlet-title .tools > a.reload:not(.one-time)', event =>  {
            event.preventDefault();
