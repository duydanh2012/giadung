class Platform {
    constructor() {
        ...
        Platform.handleCounterUp();
        Platform.initMediaIntegrate();
    ...
    static showNotice(messageType, message, messageHeader = '') {
        ...
        if (!messageHeader) {
            switch (messageType) {
                case 'error':
                    messageHeader = PlatformVariables.languages.notices_msg.error;
                    break;
                case 'success':
                    messageHeader = PlatformVariables.languages.notices_msg.success;
                    break;
            }
        }
    ...
    static handleError(data) {
        if (typeof (data.errors) !== 'undefined' && !_.isArray(data.errors)) {
            Platform.handleValidationError(data.errors);
        } else {
            if (typeof (data.responseJSON) !== 'undefined') {
                if (typeof (data.responseJSON.errors) !== 'undefined') {
                    if (data.status === 422) {
                        Platform.handleValidationError(data.responseJSON.errors);
                    }
                } else if (typeof (data.responseJSON.message) !== 'undefined') {
                    Platform.showError(data.responseJSON.message);
                } else {
                    $.each(data.responseJSON, (index, el) => {
                        $.each(el, (key, item) => {
                            Platform.showError(item);
                        });
                    });
                }
            } else {
                Platform.showError(data.statusText);
            }
        }
    }
    ...
    countCharacter() {
        $.fn.charCounter = function (max, settings) {
            ...
            settings = $.extend({
                ...
                format: '(%1 ' + PlatformVariables.languages.system.character_remain + ')',
                pulse: true,
                delay: 0
            }, settings);
            let p, timeout;
    ...
    static initResources() {
        ...
        Platform.initDatePicker('.datepicker');
        if (jQuery().mCustomScrollbar) {
            Platform.callScroll($('.list-item-checkbox'));
        }
    ...
    handlePortletTools() {
        ...
        $('body').on('click', '.portlet > .portlet-title .fullscreen', event => {
            ...
            if (portlet.hasClass('portlet-fullscreen')) {
                ...
                let height = Platform.getViewPort().height -
    ...
    static initCodeEditor(id, type = 'css') {
        ...
        $('.handle-tool-drag').mousedown(event => {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', Platform.onDragTool);
            $(window).on('mouseup', Platform.onReleaseTool);
        });
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', Platform.onDragTool);
        $(window).off('mouseup', Platform.onReleaseTool);
    }
